...
.controller('InvoiceCtrl', ['$scope', '$http', 'DEFAULT_INVOICE', 'DEFAULT_LOGO', 'LocalStorage', 'Currency',
  ...
  // Calculates the Discount of the invoice
  $scope.calculateDiscount = function() {
    return (($scope.invoice.Discount * $scope.invoiceSubTotal())/100);
  };


  // Calculates the Transportaion charges of the invoice
  $scope.calculateTrans = function() {
    return ($scope.invoice.trans);
  };
  ...
  $scope.calculateGrandTotal = function() {
    saveInvoice();
    return ( $scope.invoiceSubTotal() + $scope.calculateTax() + $scope.calculateTrans() - $scope.calculateDiscount()); 
  };
